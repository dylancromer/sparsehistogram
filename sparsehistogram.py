...
def sparse_hist(coords, resolution, use_weights, return_bin_index):
    """
    Accepts a python list of 3D spatial points, e.g. [[x1,y1,z1],...], optionally with weights e.g. [[x1,x2,x3,w1],...], and returns the sparse histogram (i.e. no empty bins) with bins of resolution (spacing) given by resolution.
    ...
    for coord, weight in coord_list:
        binned_coords[coord].append(weight)

    for coord, weight in binned_coords.items():
        weight = sum(weight)
        binned_coords.update({coord:weight})
